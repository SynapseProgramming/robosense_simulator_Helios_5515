...
pclmerger::pclmerger()
    : pub(n.advertise<sensor_msgs::PointCloud2>("final", 100)),
      twosub(n, "/two", 1), threesub(n, "/three", 1), foursub(n, "/four", 1),
      fivesub(n, "/five", 1),
      sync(TimePolicy(10), twosub, threesub, foursub, fivesub) {
  sync.registerCallback(&pclmerger::callback, this);

  std::cout << "pclmerger object has been created!" << std::endl;
}

void pclmerger::callback(const sensor_msgs::PointCloud2ConstPtr &twof,
